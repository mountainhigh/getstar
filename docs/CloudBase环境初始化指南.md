...
| 序号 | 集合名称 | 说明 | 权限 |
|------|---------|------|------|
| 1 | `families` | 家庭表 | 所有用户可读,仅创建者可写 |
| 2 | `children` | 孩子表 | 所有用户可读,仅创建者可写 |
| 3 | `habits` | 习惯表 | 所有用户可读,仅创建者可写 |
| 4 | `check_ins` | 打卡记录表 | 所有用户可读,仅创建者可写 |
| 5 | `habit_templates` | 习惯模板表 | 所有用户可读,仅创建者可写 |
| 6 | `daily_records` | 每日记录表 | 所有用户可读,仅创建者可写 |
| 7 | `badges` | 称号表 | 所有用户可读,仅创建者可写 |
| 8 | `user_badges` | 用户称号表 | 所有用户可读,仅创建者可写 |
| 9 | `rewards` | 礼物表 | 所有用户可读,仅创建者可写 |
| 10 | `reward_exchanges` | 兑换记录表 | 所有用户可读,仅创建者可写 |
| 11 | `coin_records` | 金币记录表 | 所有用户可读,仅创建者可写 |
